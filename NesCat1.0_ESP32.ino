...
#define I2S_DO_IO  (GPIO_NUM_2) //DIN
#define I2S_DI_IO  (-1)
...
#define TFT_MISO 38  // Data out (SDA) //better not change
...
#include <Arduino_GFX.h>    // Core graphics library
#include <Arduino_ILI9342.h> // Hardware-specific library for ST7789
...
Arduino_ILI9342 tft = Arduino_ILI9342(bus, TFT_RST, 0 /*rotation*/, false /*IPS*/);
...
static void lcd_write_frame(const uint16_t x, const uint16_t y, const uint16_t width, const uint16_t height) {
  ...
  for (i = 0; i < height; i++) {
    ...
    tft.draw16bitBeRGBBitmap(0, i, (uint16_t *)(SCREENBUFFER), 48, 1);
    tft.draw16bitBeRGBBitmap(48, i, (uint16_t *)(SCREENBUFFER + 48), 48, 1);
    tft.draw16bitBeRGBBitmap(96, i, (uint16_t *)(SCREENBUFFER + 96), 48, 1);
    tft.draw16bitBeRGBBitmap(144, i, (uint16_t *)(SCREENBUFFER + 144), 48, 1);
    tft.draw16bitBeRGBBitmap(192, i, (uint16_t *)(SCREENBUFFER + 192), 48, 1);
  }
}
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
   ...
   tft.begin(40000000); //max 40MHz for 1.3inch LCD!!! (80MHz only for 1.54inch LCD)
   tft.setRotation(0);
