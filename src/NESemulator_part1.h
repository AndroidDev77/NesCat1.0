...
int ret = 0;
  ...
  Serial.println("Uninstall Audio");
  //i2s_adc_enable(I2S_NUM_0);
  //i2s_set_dac_mode(I2S_DAC_CHANNEL_RIGHT_EN);
  ret = i2s_driver_install(I2S_NUM_0, &audio_cfg, 0, NULL);
  Serial.println("Set Pin: ret: " + String(ret));
...
char* NESEXPLORE(char* PATH) {
  ...
  if (!SD.open(PATH, O_READ)) {
    Serial.println("DirFile open failed");
    while (1) {};
  }
