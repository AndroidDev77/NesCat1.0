...
nes_t *nes_create(void) {
  ...
  Serial.println(">>> nes_create memsize: "+String(sizeof(nes_t)) );

  MEMORY_STATUS();

  if (NULL == machine) 
    machine = (nes_t*)malloc(sizeof(nes_t)/1);
  if (NULL == machine){ 
      Serial.println("machine alloc null");
      return NULL;
    }
  ...
  Serial.println(">>> cpu end mem init");
